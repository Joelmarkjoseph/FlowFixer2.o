...
(function(){
    ...
    try {
      ...
      if (isNEO) {
        ...
        if (apiUrl) {
          ...
          // Extract tenant URL from current page (up to /api/v1)
      ...
      const filter = "$filter=Status eq 'FAILED' and LogStart ge datetime'2025-11-14T09:28:00.000'";
  ...
  function renderInPage(rows){
    ...
    root.querySelector('#cpi-lite-resender')?.addEventListener('click', async ()=>{
      ...
      const isNEO = location.href.includes('/itspaces/');
      const keysToCheck = isNEO 
        ? ['resenderUsername', 'resenderPassword']
        : ['resenderUsername', 'resenderPassword', 'resenderApiUrl', 'resenderClientId', 'resenderClientSecret'];
      
      const savedData = await safeStorageGet(keysToCheck);
      console.log('Saved credentials check:', {
        environment: isNEO ? 'NEO' : 'CF',
        hasUsername: !!savedData.resenderUsername,
        hasPassword: !!savedData.resenderPassword,
        hasApiUrl: !!savedData.resenderApiUrl,
        hasClientId: !!savedData.resenderClientId,
        hasClientSecret: !!savedData.resenderClientSecret
      });
      
      // Check if all required credentials are present
      const hasAllCredentials = isNEO
        ? (savedData.resenderUsername && savedData.resenderPassword)
        : (savedData.resenderUsername && savedData.resenderPassword && savedData.resenderApiUrl && savedData.resenderClientId && savedData.resenderClientSecret);
      
      if (hasAllCredentials) {
        // Auto-connect with saved credentials
        status.textContent = 'Loading resender overview...';
        console.log('Using saved credentials to fetch resender overview');
        try{
          const iflowSummary = await fetchResenderOverview(savedData.resenderUsername, savedData.resenderPassword);
          const main = findMainContentContainer();
          if (main){
            renderResenderOverviewPageFull(iflowSummary, savedData);
            activateFullPageMode();
          } else {
            renderResenderOverviewPagePanel(iflowSummary, savedData);
          }
          status.textContent = `Loaded overview for ${iflowSummary.length} iFlows`;
        }catch(e){
          const errorMsg = String(e && e.message || e);
          console.error('Resender overview failed with saved credentials:', errorMsg);
          
          // Check if it's an authentication error
          if (errorMsg.includes('401') || errorMsg.includes('403') || errorMsg.includes('Authentication')) {
            status.textContent = 'Authentication failed - please re-enter credentials';
          } else {
            status.textContent = 'Failed to load: ' + errorMsg;
          }
          
          // If saved credentials fail, show dialog
          showAuthDialogForResender(async (url, username, password, apiUrl)=>{
            status.textContent = 'Fetching message processing logs...';
            try{
              await fetchMessageProcessingLogs(username, password, apiUrl);
              status.textContent = 'Message processing logs fetched - check console';
            }catch(e){
              status.textContent = String(e && e.message || e);
  ...
  function renderFullPage(rows){
    ...
    root.querySelector('#cpi-lite-resender')?.addEventListener('click', async ()=>{
      ...
      const savedData = await safeStorageGet(keysToCheck);
      console.log('[Full Page] Saved credentials check:', {
        environment: isNEO ? 'NEO' : 'CF',
        hasUsername: !!savedData.resenderUsername,
        hasPassword: !!savedData.resenderPassword,
        hasApiUrl: !!savedData.resenderApiUrl,
      ...
      if (hasAllCredentials) {
        ...
        try{
          ...
          const errorMsg = String(e && e.message || e);
          console.error('[Full Page] Resender overview failed with saved credentials:', errorMsg);
          ...
          showAuthDialogForResender(async (url, username, password, apiUrl)=>{
            ...
            try{
              ...
              status.textContent = String(e && e.message || e);
              alert('Failed to load resender overview: ' + String(e && e.message || e));
            }
          });
        }
      } else {
